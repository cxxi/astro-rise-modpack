...
check_consistency() {
    ...
    if [ ! -d "$asset_dir" ]; then
        if [[ "$title" == "Resource Packs" || "$title" == "Datapacks" ]]; then
             if [ ! -d "$asset_dir" ]; then
                echo -e "${COLOR_YELLOW}Info: Directory '$asset_dir' not found. Assuming empty, continuing check.${COLOR_RESET}"
             fi
        else
            if [ ! -d "$asset_dir" ]; then
                echo -e "${COLOR_RED}Error: Required directory '$asset_dir' not found. Skipping check.${COLOR_RESET}\n"
                return
            fi
        fi
    fi
